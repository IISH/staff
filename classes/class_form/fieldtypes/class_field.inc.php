...
class class_field {
	...
	function push_field_into_query_array($query_fields) {
		...
		array_push($query_fields, array($this->getFieldname() => $value));
	...
	function get_form_value($field = '' ) {
		if ( $field == '' ) {
			$retval = $_POST["FORM_" . $this->getFieldname()];
		} else {
	...
	function setInputFieldAttributes($inputfield) {
		$inputfield = str_replace("::FIELDNAME::", $this->getFieldname(), $inputfield);
